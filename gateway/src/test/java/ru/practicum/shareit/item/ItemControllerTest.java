...
public class ItemControllerTest {
    ...
    @Test
    void addNewItemNameError() throws Exception {
        ...
        verify(itemClient, never()).add(eq(1L), any(ItemDto.class));
    }

    @Test
    void addNewItemDescError() throws Exception {
    ...
    @Test
    void addNewItemAvailError() throws Exception {
    ...
    void findByIdOk() throws Exception {
        when(itemClient.findById(anyLong(), anyLong())).thenReturn(ResponseEntity.ok(itemDto));
    ...
    void findAllOk() throws Exception {
        when(itemClient.findAll(anyLong())).thenReturn(ResponseEntity.ok(List.of(itemDto)));

        mockMvc.perform(get("/items")
                        .header(ItemController.USER_ID_HEADER, "1"))
                .andExpect(status().isOk());

        verify(itemClient, times(1)).findAll(anyLong());
    }
    ...
    void updateItemOk() throws Exception {
        ...
        verify(itemClient, times(1)).update(eq(1L), eq(1L), eq(itemDto));
    }
